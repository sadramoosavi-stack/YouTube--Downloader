...
from source.schemas import DownloadRequest, DownloadResponse, DownloadHistoryResponse
...
def create_download(data: DownloadRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        if data.file_type == "video":
            ...
            ydl_options = {"format": format_option, "outtmpl": str(download_folder / "%(title)s.%(ext)s"), "merge_output_format": "mp4", "noplaylist": True, "restrictfilenames": True}

        else:
            ydl_options = {"format": "bestaudio/best", "outtmpl": str(download_folder / "%(title)s.%(ext)s"), "noplaylist": True, "restrictfilenames": True, "postprocessors": [{"key": "FFmpegExtractAudio", "preferredcodec": "mp3", "preferredquality": "192"}]}
...
def remove_download(download_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    download_folder = DOWNLOAD_FOLDER / str(download.id)

    if download_folder.exists():
        try:
            shutil.rmtree(download_folder)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Could not delete downloaded file: {str(e)}")
